...
`.github/workflows/validate.yml` 目前只在 `develop` 分支推送時觸發；原始碼中的合併請求觸發設定目前被註解。

- Rust 工作在 Linux、Windows 與 macOS 矩陣執行。
- 前端工作在 Linux、Windows 與 macOS 矩陣執行。
- Rust 工作先準備格式化工具與靜態檢查工具，再執行 `bash .neo_harness/verify.sh rust`。
- 前端工作使用 Node.js 20 與鎖定檔安裝依賴，再執行 `bash .neo_harness/verify.sh frontend`。
